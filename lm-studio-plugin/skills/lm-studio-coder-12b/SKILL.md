---
name: lm-studio-coder-12b
description: >-
  Query local google/gemma-4-12b for code generation tasks. Enforces writing directly to files. Automatically JIT loads the model if necessary.
---

# LM Studio Coder 12B Skill

## Overview
This skill allows agents to use the local `google/gemma-4-12b` model for code generation, refactoring, and code analysis. It interacts with the local server running at `http://localhost:1234/v1`.
...
```bash
python3 ~/.gemini/config/plugins/lm-studio-plugin/scripts/lm_studio_cli.py query --model google/gemma-4-12b --code-only --prompt "Write a Python script to parse JSON." --output src/parser.py
```
