---
name: lm-studio-chat-12b
description: >-
  Query local google/gemma-4-12b for reasoning, general chat, summarization, and log diagnosis. Automatically JIT loads the model if necessary.
---

# LM Studio Chat 12B Skill

## Overview
This skill allows agents to use the local `google/gemma-4-12b` model for text generation, reasoning, and summarizing large logs. It interacts with the local server running at `http://localhost:1234/v1`.
...
- `--model` (MANDATORY): You must specify `--model google/gemma-4-12b`
...
```bash
python3 ~/.gemini/config/plugins/lm-studio-plugin/scripts/lm_studio_cli.py query --model google/gemma-4-12b --prompt "Identify the root cause of the error in this log and provide a 1 sentence fix: @/tmp/server.log" --output /tmp/diagnosis.txt
```
